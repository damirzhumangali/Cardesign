...
const MODEL_CONFIGS = [
  {
    ...
    path: '/models/lamborghini_aventador.glb',
    targetSize: 3.15,
    introOffset: {
      x: 0,
      z: 0,
  ...
  },
  {
    ...
    path: '/models/mercedes-amg_gt3__www.vecarz.com.glb',
    targetSize: 3.35,
    introOffset: {
      x: 0.9,
      z: -0.35,
...
function applyEntryState(entry, visibility, blend, isIncoming) {
  ...
  entry.container.position.x = THREE.MathUtils.lerp(
    introOffset.x,
    exitOffset.x,
    blend,
  );
  entry.container.position.z = THREE.MathUtils.lerp(
    introOffset.z,
    exitOffset.z,
    blend,
  );
